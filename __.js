...
var __ = {
    ...
    isValidDate : function(value) {
        ...
        value = value.replace(/ AM/ig,'').replace(/ PM/ig, '');
        var matches = value.match(/^(\d{1}|\d{2}).(\d{1}|\d{2}).(\d{4}).(\d{1}|\d{2}).(\d{1}|\d{2}).(\d{1}|\d{2})$/);
        //alt:
    ...
    ****** */
    model : {
        ...
        */
            is_array : function(x) {
                return this.Collection.is_array(x);
            },
            size : function(Collection) {
                return this.Collection.size(Collection);
            },
            set : function(objOptions) {
                return this.Collection.set(objOptions);
            },
            get : function(Collection, where, log) {
                return this.Collection.get(Collection, where, log);
            },
            log : function(Collection) {
                return this.Collection.log(Collection);
            },
            isValid : function(Collection, Prototype) {
                return this.Collection.isValid(Collection, Prototype);
            },
            sortBy : function(Collection, keys, reverse, primer) {
                return this.Collection.sortBy(Collection, keys, reverse, primer);
            },
            valueExists : function(Collection, where) {
                return this.Collection.valueExists(Collection, where);
            },
            search : function(objOptions) {
                return this.Collection.search(objOptions);
            },
            each : function(collection, callback, where, offset, rowCount) {
                return this.Collection.forEach(collection, callback, where, offset, rowCount);
            },
            insertInto : function(Collection, KeyValue) {
                return this.Collection.insertInto(Collection, KeyValue);
            },
            append : function(Collection, options) {
                return this.Collection.append(Collection, options);
            },
            update : function(Collection, where, newKeys) {
                return this.Collection.update(Collection, where, newKeys);
            },
            deleteFrom : function(Collection, where) {
                return this.Collection.deleteFrom(Collection, where);
            },
        /*
        *
        *** */
        ...
        remove : function(options) {
            models = ( options instanceof Array) ? options : [options];

            for ( var i=0; i<models.length; i++) {
                this.Collection.data[models[i]] = null;
                delete(this.Collection.data[models[i]]);
            }
        ...
        logDataModels : function() {
            console.log('____________________________');
            for ( var prop in this.Collection.data) {
                console.group(prop);
                console.log(this.Collection.data[prop]);
                console.groupEnd();
            }
        },
        ...
        removeAll : function(options) {
            ...
            for ( var prop in self.data) {
                if ( !models.hasOwnProperty(prop) ) {
                    this.Collection.data[prop] = null;
                    delete(this.Collection.data[prop]);
                }
            }
        },
        ...
        addToLocalStorage :function(options) {
        ...
        getFromLocalStorage :function(options) {
                for ( var i=0; i<options.length; i++ ) {
                    try {
                        localStorageItem = JSON.parse( sessionStorage.getItem( options[i] ) );
                    } catch(e) {
                        localStorageItem = {};
                    }
        ...
        getLocalStorageCreateDate : function(options) {
            try {
                localStorageItem = JSON.parse( sessionStorage.getItem( options ) );
                return localStorageItem.datetime;
        ...
        Collection : {
            ...
            */
            set : function(objOptions) {
                ...
                if ( options.model instanceof Object || options.model instanceof Array) {
                    this.data[options.name] = options.model;
                } else {
                    ...
                    if ( typeof where === 'object' ) {
                        if ( where.hasOwnProperty('key') ) {
                            ...
                            for ( var prop in collection ) {
                                if ( collection[prop] == where.value ) {
                                    return prop;
                                }
                            }
                        }
                ...
                if ( collection instanceof Array ) {
                    ...
                    var keySort = function(a, b, d) {
                        ...
                        if (__.isValidDate(a))
                        {
                            ...
                            a = (typeof a !== 'undefined')? ( __.isNumeric(a)) ? a : a.toLowerCase() : "";
                            b = (typeof b !== 'undefined')? ( __.isNumeric(b)) ? b : b.toLowerCase() : "";
                            a = (a=="")?null:a;
                            b = (b=="")?null:b;
                        }
            ...
            */
            search : function(objOptions) { //Collection, where, notIn, options) {
                ...
                if ( typeof options.Where === 'object' ) {
                    ...
                    options.Where.like = (options.Where.hasOwnProperty('like')) ? options.Where.like : true;
                ...
                if ( KeywordsList == '' && !options.Evals instanceof Array ) {
                ...
                this.forEach(options.Collection, function(Item, Index) {                    
                    ...
                    var ignoreIndex = ( __.model.Collection.is_array(NotInList) && NotInList.length > 0 && ( NotInList.indexOf(Item[CollectionKey]) > -1 ) );
                    ...
                    if ( !ignoreIndex ) {
                            ...
                            if ( KeywordsList === null ) {
                                ...
                                for ( i=0; i<KeywordsList.length; i++ ) {
                                    k = KeywordsList[i];
                                    k = ( k instanceof String ) ? k.toLowerCase() : k;
                                    KeywordsListReference[k] = 0;
                                }
                                ...
                                for (i=0; i<KeywordsList.length; i++) {
                                    ...
                                    //if ( keyword != "" ) {
                                        ...
                                        //
                                        // A needle was specified, so only search that property
                                        if ( typeof needle !== 'undefined' ) {
                                            if ( options.Where.like && Item[needle].toLowerCase().indexOf(keyword) > -1 ) {
                                                KeywordsListReference[keyword] = 1;
                                            } else if ( !options.Where.like && Item[needle].toLowerCase() == keyword) {
                                                KeywordsListReference[keyword] = 1;
                                            }
                            ...
                            if ( options.Evals instanceof Array ) {
                                ...
                                for ( var prop in options.Evals ) {
                                    ...
                                    var Key   = _Eval.Key;
                                    if ( _Eval.Operator != '=' ) {
                                        equation = '(Item["' + _Eval.Key + '"].toLowerCase() ' + _Eval.Operator;
                                        equation += ( __.isNumeric(_Eval.Value)) ? _Eval.Value : " '" + _Eval.Value.toLowerCase() + "'";
            ...
            */
            ...
            /**
                ...
                __.model.Collection.update(
                    'nameOfCollection',{
                        index : [1234|'PropertyName'],
                        equals : 'NewValueOfProperty'
                    }
                )
            */
            update : function(Collection, where, newKeys) {
                ...
                if (where instanceof Array) {
                    for ( var i = 0; i < where.length; i++ ) {
                        __.model.Collection.updateWhereClause(collection, where[i], newKeys);
                    }
                } else {
                    __.model.Collection.updateWhereClause(collection, where, newKeys);
                }
            },

            updateWhereClause : function(Collection, where, newKeys) {
                collection = this.returnCollection(Collection);
                ...
                } else if ( typeof(where) !== 'undefined' || (where.hasOwnProperty('key') && where.hasOwnProperty('equals')) ) {
                    needle = where['key'];
                    val    = where['equals'];
            ...
            */
            deleteFrom : function(Collection, where) {
                ...
                if (where instanceof Array) {
                    for ( var i = 0; i < where.length; i++ ) {
                        __.model.Collection.deleteFromWhereClause(collection, where[i]);
                    }
                } else {
                    __.model.Collection.deleteFromWhereClause(collection, where);
                }
            },

            deleteFromWhereClause : function(Collection, where) {
                collection   = this.returnCollection(Collection);

                if ( where instanceof Object ) {
                    needle = (where.hasOwnProperty('key')) ? where['key'] : undefined;
                    equals = (where.hasOwnProperty('equals')) ? where['equals'] : undefined;
                    is     = (where.hasOwnProperty('is')) ? where['is'] : undefined;

                    if ( collection instanceof Object ) {
                        if ( typeof equals !== 'undefined' ) {
                            for ( i=0; i<collection.length; i++) {
                                if ( collection[i].hasOwnProperty(needle) && collection[i][needle] === equals ) {
                                    collection.splice(i,1);
                                }
                            }
                        }
                        else if ( typeof is !== 'undefined' ) {
                            if ( typeof collection[needle] !== 'undefined' ) {
                                if ( typeof collection[needle][is] !== 'undefined' )
                                    delete(collection[needle][is]);
                            }
                            else if ( collection.hasOwnProperty[needle] ) {
                                if ( collection[needle].hasOwnProperty(is) )
                                    delete(collection[needle][is]);
                            }
                        }
                    }
    ...
    ********************************************** */
    ...
    forEach : function(collection, callback, where, offset, rowCount) {
        return __.model.Collection.forEach(collection, callback, where, offset, rowCount);
    },

    each : function(collection, callback, where, offset, rowCount) {
        return this.forEach(collection, callback, where, offset, rowCount);
    },

    // for : function(collection, callback, where, offset, rowCount) {
    //  return this.forEach(collection, callback, where, offset, rowCount);
    // },
    ...
    ********************************************** */
    ...
    POST : function(url, request, return_response) {
        if ( typeof request === 'undefined' ) {
            request = {};
        }
        if ( typeof return_response === 'undefined' ) {
        ...
        $.ajax({
            ...
            , error: function (jqXHR, textStatus, errorThrown) {
                $.modal({
                    ...
                    message : "<section><p>The server returned a " + jqXHR.status + " error.</p><p>Error Message: " + jqXHR.responseText + "</p></section>"
                });
            }
            , success : function(response, status, xhr) {
                ...
                *       errors  : {
                *             "Company"  : "Required",
                *             "WellNane" : "Company Name is Required"
                *       },
                *       results : {}
                *     }
                *
                *   List of standard error responses
                ...
                */
                if ( response == null ) {
                    ...
                    } else if ( !response.hasOwnProperty('code') && !response.hasOwnProperty('status') ) {
                        response = DefaultResponse;
                    }
                ...
                if (typeof response.results == "string") {
                    try
                    {
                        response.results = JSON.parse(this.responseText);
                    }
                    catch(e)
                    {
                ...
                //
                // Handle errors
                errors.handler(response);
                ajaxResponse = (!return_response) ? response.results : response;
    ...
    removeStopWords : function(FullString) {
        ...
        if ( FullString instanceof String ) {
...
*/
    var Queue = function() {
        var $self = this;
        $self._q_timeout  = undefined;
        $self._stack = [];
        $self.response = undefined;


        $self.when = function(fn) { 
            if ( $self._q_timeout == undefined ) {
                $self._q_timeout = window.setTimeout(fn, 0);
            } else {
                $self._stack.push(fn);
            }
            return $self;
        ...
        $self.deny = function(err) {
            var trace = printStackTrace();
            //Output however you want!
            console.log(trace.join('\n'));
            console.log('There was an error: ' + err.message);
            console.log('Next process in stack: ' + $self._stack.shift());
            return $self;
        },
        ...
        $self.resolve = function(resp) {
            $self._response = resp;
            var stacksLeft = $self._stack.length;
            if ( stacksLeft > 0 ) {
                // try {
                    $self._stack.shift()(resp);
              //   } catch(err) {
                    // console.log('There was an error: ' + err.message);
                    // console.log($self._stack);
              //   }
            }
            if ( stacksLeft <= 0 ) {
                $self.finish(resp);
